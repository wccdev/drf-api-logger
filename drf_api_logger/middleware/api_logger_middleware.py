...
class APILoggerMiddleware:
    def __init__(self, get_response):
        ...
        if hasattr(settings, "DRF_API_LOGGER_PATH_TYPE"):
            if settings.DRF_API_LOGGER_PATH_TYPE in [
                "ABSOLUTE",
                "RAW_URI",
                "FULL_PATH",
            ]:
        ...
        if hasattr(settings, "DRF_API_LOGGER_SKIP_NAMESPACE"):
            if (
                ...
                self.DRF_API_LOGGER_SKIP_NAMESPACE = (
                    settings.DRF_API_LOGGER_SKIP_NAMESPACE
                )

        self.DRF_API_LOGGER_METHODS = []
        if hasattr(settings, "DRF_API_LOGGER_METHODS"):
            if (
                type(settings.DRF_API_LOGGER_METHODS) is tuple
                or type(settings.DRF_API_LOGGER_METHODS) is list
            ):
    ...
    def __call__(self, request):
        ...
        if (
            ...
            and request.method not in self.DRF_API_LOGGER_METHODS
        ):
            return self.get_response(request)
        ...
        if getattr(response, "streaming", False):
            ...
            if hasattr(response, "_rendered_data"):
                ...
                response_body = response._rendered_data  # noqa
        ...
        data = dict(
            ...
            result_code=get_result_code(
                response_body, self.DRF_API_LOGGER_RESULT_CODE_KEY
            )
